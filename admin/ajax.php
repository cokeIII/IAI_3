...
if ($_POST["act"] == "changePass") {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        $techlist["data"][$i]["start_end_date"] = "เริ่ม " . DateThai($dateArrStrat[0])." ". $dateArrStrat[1] . "<br> จบ " . DateThai($dateArrEnd[0])." ". $dateArrEnd[1];
        $techlist["data"][$i]["location"] = $row["location"];
        $techlist["data"][$i]["btnTable"] = '<a href="course_table_form.php?course_id=' . $row["course_id"] . '"><button class="btn btn-primary text-nowrap" course_id="' . $row["course_id"] . '">จัดตาราง</button></a>';
        $techlist["data"][$i]["btnLecturer"] = '<a href="add_lec.php?course_id=' . $row["course_id"] . '" class="btn btn-info btnLecturer text-nowrap">เพิ่มวิทยากร</a>';
        $techlist["data"][$i]["btnForm"] = '<a href="create_form.php?course_id=' . $row["course_id"] . '" class="btn btn-info btnLecturer text-nowrap">สร้างฟอร์มเพิ่ม</a>';
        $i++;
    }
